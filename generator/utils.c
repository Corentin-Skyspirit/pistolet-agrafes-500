...
void* xmalloc(size_t sz) {
	...
	if (!out) {
		fprintf(stderr, "Out of memory trying to allocate %zu byte(s) at %s:%d (%s)\n", sz, __FILE__, __LINE__, __func__);
		abort();
	}
	return out;
}
void* xcalloc(size_t n, size_t sz) {
	...
	if (numa_avail) {
		size_t to_alloc = n * sz;
		if (to_alloc < n || to_alloc < sz) {
			fprintf(
				stderr, "Allocation size out of range for %zu items of %zu byte(s) at %s:%d (%s)\n", n, sz, __FILE__, __LINE__, __func__);
			abort();
		}
		out = numa_alloc(n * sz);
#	if defined(_OPENMP)
...
#	endif
	...
	if (!out) {
		fprintf(
			stderr, "Out of memory trying to allocate/clear %zu items of %zu byte(s) at %s:%d (%s)\n", n, sz, __FILE__, __LINE__, __func__);
		abort();
	}
	return out;
}
